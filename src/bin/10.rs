...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    let total_score: u32 = (0..rows).into_iter().map(|i| {
        (0..cols)
            .filter(|&j| grid[i][j] == 0)
...
fn count_reachable_nines(grid: &Grid, start_i: usize, start_j: usize, rows: usize, cols: usize) -> u32 {
...
pub fn part_two(input: &str) -> Option<u32> {
    ...
    let total_rating: u32 = (0..rows).into_iter().map(|i| {
        (0..cols)
            .filter(|&j| grid[i][j] == 0)
...
fn count_distinct_paths(grid: &Grid, start_i: usize, start_j: usize, rows: usize, cols: usize) -> u32 {
    ...
    fn dfs(
        ...
        if i > 0 && !current_path.contains(&(i-1, j)) && grid[i-1][j] == height + 1 {
            dfs(i-1, j, grid, path_count, current_path, height + 1, rows, cols);
        }
        if j > 0 && !current_path.contains(&(i, j-1)) && grid[i][j-1] == height + 1 {
            dfs(i, j-1, grid, path_count, current_path, height + 1, rows, cols);
        }
    ...
    dfs(start_i, start_j, grid, &mut path_count, &mut current_path, 0, rows, cols);
    path_count
}
